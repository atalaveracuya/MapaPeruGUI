...
clear all
global shapefile  "E:\ANDRES\Documents\GitHub\MapaPeruGUI\shapefile"
global dofile     "E:\ANDRES\Documents\GitHub\MapaPeruGUI\dofile"
global dataset    "E:\ANDRES\Documents\GitHub\MapaPeruGUI\dataset"
global graficos   "E:\ANDRES\Documents\GitHub\MapaPeruGUI\graficos"
...
use "xydatabase.dta", clear
#delimit ;
spmap using "xycoord.dta", id(_ID) ocolor(black) 
...
#delimit cr	
graph export "${graficos}//map1.svg", width(1000) replace
...
#delimit cr	
graph export "${graficos}//map2.svg", width(1000) replace
...
spmap using "xy_coor_with_squares.dta", id(_ID) ocolor(gs6) 
polygon(data("xycoor_titicaca.dta") osize(0.3) fcolor(blue) ocolor(blue)) line(data("xy_coor_with_squares.dta") size(0.2) color(black) pattern(shortdash))
    point(data("xy_dbase_ubicaciones.dta") xcoord(x_anchor) ycoord(y_anchor) shape(x) ocolor(white) ) ;
#delimit cr	
graph export "${graficos}//map3.png", width(1000) replace
...
#delimit ;
grmap pc using "coormzn-150132.dta", id(_ID) ocolor(none ..) mosize(0.001)
    note("Zona Censal 07000 San Juan de Lurigancho , Lima. Shapefile tomado de https://www.geogpsperu.com/", size(*0.7) color(black))
    fcolor("255 251 218" "255 236 147" "250 195 103" "234 96 24" "229 27 27")
    osize(0.01)
    legenda(on)
    legstyle(3)
    ...
    legend(label(2 "0.1 - 9.9 (`leg0')") label(3 "10.0 - 19.9 (`leg1')")  label(4 "20.0 - 39.9 (`leg2')")  label(5 "40 - 59.9 (`leg3')") label(6 "60.0 - `fmt_max' (`leg4')") margin(1 1 1 1));
#delimit cr	
graph export "${graficos}//map11.png", width(1000) replace

***


*Exportar a geogson 
geojson save geovar using " zona0700-SJL", features("Polygon")
